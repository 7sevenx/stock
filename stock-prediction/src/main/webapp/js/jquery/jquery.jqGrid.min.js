	...
	$.fn.jqGrid = function(pin) {
		...
		return this.each(function() {
			...
			var sortkeys = [ "shiftKey", "altKey", "ctrlKey" ], IntNum = function(val, defval) {
							...
							complete : function(req, st) {
								if (st == "success" || (req.statusText == "OK" && req.status == "200")) {
									if (dt === "xml") {
										...
										addJSONData($.jgrid.parse(req.responseText), ts.grid.bDiv, rcnt, npage > 1, adjust)
									}
									if (lc) {
										lc.call(ts, req)
									}
				...
				var sep = "<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='ui-separator'></span></td>", pgid = $(ts.p.pager).attr("id") || "pager", pginp = "", pgl = "<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>", str = "", pgcnt, lft, cent, rgt, twd, tdw, i, clearVals = function(onpaging) {
				...
				cent = pgid + "_center";
				rgt = pgid + "_right";
				$(ts.p.pager).addClass("ui-jqgrid-pager corner-bottom").append("<div id='" + pgcnt + "' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;' role='row'><tbody><tr><td id='" + lft + "' align='left'></td><td id='" + cent + "' align='center' style='white-space:pre;'></td><td id='" + rgt + "' align='right'></td></tr></tbody></table></div>").attr("dir", "ltr");
			...
			for (i = 0; i < this.p.colNames.length; i++) {
				thead += "<th role='columnheader' class='theme-tc ui-state-default ui-th-column ui-th-" + dir + "'>";
					...
					if (this.checked) {
						...
						$(ts.rows).each(function(i) {
							if (!$(this).hasClass("subgrid")) {
								$(this).addClass("ui-state-highlight").attr("aria-selected", "true").addClass("theme-vice");
						...
						$(ts.rows).each(function(i) {
							if (!$(this).hasClass("subgrid")) {
								$(this).removeClass("ui-state-highlight").attr("aria-selected", "false").removeClass("theme-vice");
								emp[i] = this.id
							}
						});
						ts.p.selarrrow = [];
			...
			if (ts.p.pager) {
				...
				$(ts.p.pager).css({
					width : grid.width + "px"
				}).appendTo(eg).addClass("ui-state-default ui-jqgrid-pager");
			...
			return this.each(function() {
				...
				if (!$t.p.multiselect) {
					if ($(pt).attr("class") !== "subgrid") {
						if ($t.p.selrow) {
							$("tr#" + $.jgrid.jqID($t.p.selrow), $t.grid.bDiv).removeClass("ui-state-highlight").attr("aria-selected", "false").removeClass("theme-vice")
						}
						$t.p.selrow = pt.id;
						$(pt).addClass("ui-state-highlight").attr("aria-selected", "true").addClass("theme-vice");
					...
					if (ia === -1) {
						...
						if ($(pt).attr("class") !== "subgrid") {
							$(pt).removeClass("ui-state-highlight").attr("aria-selected", "false").removeClass("theme-vice")
						}
						stat = false;
						$("#jqg_" + $.jgrid.jqID($t.p.selrow), $t.rows).attr("checked", stat);
			...
			return this.each(function() {
				...
				if (!t.p.multiselect) {
					...
					$(t.p.selarrrow).each(function(i, n) {
						...
						$(ind).removeClass("ui-state-highlight").attr("aria-selected", "false").removeClass("theme-vice");
						$("#jqg_" + $.jgrid.jqID(n), ind).attr("checked", false)
					});
				...
				if (pos >= 0) {
					...
					if (ln && ln > 0) {
						while (i < ln) {
							...
							mathopr ? sum += parseFloat(val, 10) : obj ? ret.push({
								id : $t.rows[i].id,
								value : val
							}) : ret[i] = val;
							i++
	...
	if (b.resize) {
		if (jQuery.fn.jqResize) {
			...
			try {
				jQuery(k).resizable({
					handles : "se, sw",
					alsoResize : l.scrollelm ? "#" + l.scrollelm : false
				})
			} catch (f) {
			}
		}
	...
	case "custom":
		...
		try {
			if (jQuery.isFunction(f.custom_element)) {
				...
				if (a) {
					a = jQuery(a).addClass("customelement").attr({
						id : f.id,
						name : f.name
					});
	...
	b.jgrid.extend({
				...
				if (b.fn.searchFilter) {
					...
					if (b("#" + g).html() != null) {
						...
						b.each(s, function(w, j) {
							q = (typeof j.search === "undefined") ? true : j.search, hidden = (j.hidden === true), soptions = b.extend({}, {
								text : v[w],
								itemval : j.index || j.name
							}, this.searchoptions), ignoreHiding = (soptions.searchhidden === true);
						...
						if (n.length > 0) {
							...
							b("#" + g).searchFilter(n, {
								...
								ajaxSelectOptions : b.extend({}, b.jgrid.ajaxOptions, l.p.ajaxSelectOptions || {})
							});
							...
							if (c.drag === true) {
								...
								if (jQuery.fn.jqDrag) {
									...
									try {
										b("#" + g).draggable({
											handle : b("#" + g + " table thead tr:first td:first")
										})
									} catch (o) {
									}
								}
		...
		editGridRow : function(c, d) {
			...
			return this.each(function() {
				...
				var C = e.p.id, y = "FrmGrid_" + C, u = "TblGrid_" + C, i = {
					themodal : "editmod" + C,
					modalhead : "edithd" + C,
					modalcontent : "editcnt" + C,
					scrollelm : y
				}, D = b.isFunction(a.beforeShowForm) ? a.beforeShowForm : false, O = b.isFunction(a.afterShowForm) ? a.afterShowForm : false, N = b.isFunction(a.beforeInitData) ? a.beforeInitData : false, o = b.isFunction(a.onInitializeForm) ? a.onInitializeForm : false, I = null, J = 1, q = 0, v, E, F, R, H, B;
				if (c == "new") {
					c = "_empty";
				...
				function s(ab, ah, ae, am) {
					...
					b(ah.p.colModel).each(function(au) {
						...
						if (Y !== "cb" && Y !== "subgrid" && this.editable === true && Y !== "rn") {
							if (af === false) {
								ao = ""
							} else {
								if (Y == ah.p.ExpandColumn && ah.p.treeGrid === true) {
									...
									try {
										ao = b.unformat(b("td:eq(" + au + ")", ah.rows[af]), {
											colModel : this
										}, au)
									} catch (ar) {
							...
							var at = b.extend({}, this.editoptions || {}, {
								id : Y,
								name : Y
							});
							frmopt = b.extend({}, {
								...
								rowabove : false,
								rowcontent : ""
							}, this.formoptions || {}), ag = parseInt(frmopt.rowpos) || ak + 1, aa = parseInt((parseInt(frmopt.colpos) || 1) * 2);
				...
				function n(Z, af, ab) {
					...
					b("td", aa).each(function(an) {
						ak = aj[an].name;
						if (ak !== "cb" && ak !== "subgrid" && ak !== "rn" && aj[an].editable === true) {
				...
				function j() {
					...
					if (!d.processing) {
						...
						b.ajax(b.extend({
							url : v,
							type : a.mtype,
							data : b.isFunction(a.serializeEditData) ? a.serializeEditData(E) : E,
							complete : function(ae, ad) {
								if (ad != "success") {
									aa[0] = false;
		...
		viewGridRow : function(c, d) {
			...
			return this.each(function() {
				...
				var r = x.p.id, A = "ViewGrid_" + r, s = "ViewTbl_" + r, j = {
					themodal : "viewmod" + r,
					modalhead : "viewhd" + r,
					modalcontent : "viewcnt" + r,
					scrollelm : A
				}, h = 1, f = 0;
				...
				function n(L, R, P, X) {
					...
					b(R.p.colModel).each(function(ae) {
						...
						if (H !== "cb" && H !== "subgrid" && H !== "rn") {
							...
							var ad = b.extend({}, this.editoptions || {}, {
								id : H,
								name : H
							}), ai = b.extend({}, {
								rowabove : false,
								rowcontent : ""
							}, this.formoptions || {}), af = parseInt(ai.rowpos) || V + 1, ah = parseInt((parseInt(ai.colpos) || 1) * 2);
							if (ai.rowabove) {
						...
						if (q[0] === false) {
							...
							if (!d.processing) {
								...
								b.ajax(b.extend({
									url : p,
									type : d.mtype,
									data : b.isFunction(d.serializeDelData) ? d.serializeDelData(t) : t,
...
(function(a) {
	a.jgrid.extend({
		editRow : function(c, i, h, j, b, e, d, f, g) {
			return this.each(function() {
				var n = this, s, o, l, m = 0, r = null, q = {}, k, p;
				if (!n.grid) {
					return
				}
					...
					if (m > 0) {
						...
						if (i === true) {
							a(k).bind("keydown", function(t) {
								if (t.keyCode === 27) {
									a(n).jqGrid("restoreRow", c, g)
								}
								if (t.keyCode === 13) {
									a(n).jqGrid("saveRow", c, j, b, e, d, f, g);
									return false
								}
								t.stopPropagation()
							})
			...
			return this.each(function() {
				...
				if (j === "1" && e) {
					...
					a("td", i).each(function(v) {
						t = o.p.colModel[v];
						u = t.name;
						if (u != "cb" && u != "subgrid" && t.editable === true && u != "rn") {
							switch (t.edittype) {
							case "checkbox":
								var k = [ "Yes", "No" ];
								if (t.editoptions) {
					...
					if (!o.grid.hDiv.loading) {
						...
						if (e == "clientArray") {
							...
							a.ajax(a.extend({
								url : e,
								data : a.isFunction(o.p.serializeRowData) ? o.p.serializeRowData(p) : p,
								type : "POST",
						...
						if (j[0] === true) {
							...
							if (k.p.cellsubmit == "remote") {
								if (k.p.cellurl) {
									...
									a.ajax(a.extend({
										url : k.p.cellurl,
										data : a.isFunction(k.p.serializeCellData) ? k.p.serializeCellData(o) : o,
										type : "POST",
										complete : function(e, v) {
											if (v == "success") {
					...
					if (!m.grid.hDiv.loading) {
						...
						case "json":
							a.ajax(a.extend({
								type : m.p.mtype,
								url : m.p.subGridUrl,
								dataType : m.p.subgridtype,
								data : a.isFunction(m.p.serializeSubGridData) ? m.p.serializeSubGridData(v) : v,
								complete : function(w) {
				...
				var e = function(h, k) {
					if (h && typeof h == "string") {
						...
						if (i) {
							...
							l.datastr = i;
							a(f).jqGrid(l).jqGrid("setGridParam", {
								datatype : j
							})
						} else {
							a(f).jqGrid(l)
			...
			return this.each(function() {
				...
				opts.stop = function(ev, ui) {
					$($t).jqGrid("setGridParam", {
						height : $("#gview_" + $t.p.id + " .ui-jqgrid-bdiv").height()
					});
